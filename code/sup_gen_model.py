...
##########################################
# different from gen_model.py, adding supervised loss, i.e., add a cross entropy loss
##########################################
class SUP_GEN():
    def __init__(self, itemNum, userNum, emb_dim, lamda, dpp_lamda, param=None, initdelta=0.05, learning_rate=0.05):
        ...
        self.sup_labels = tf.placeholder(tf.int32)
        self.sup_ids = tf.placeholder(tf.int32)

        self.u_embedding = tf.nn.embedding_lookup(self.user_embeddings, self.u)
        self.i_embedding = tf.nn.embedding_lookup(self.item_embeddings, self.i)
        self.sup_i_embedding = tf.nn.embedding_lookup(self.item_embeddings, self.sup_ids)
        ...
        self.gan_loss = -tf.reduce_mean(tf.log(self.i_prob) * self.reward)  ##rewards are negative
        self.norm_loss = self.lamda * (
            tf.nn.l2_loss(self.u_embedding) + tf.nn.l2_loss(self.i_embedding) + tf.nn.l2_loss(self.i_bias))
        
        self.sup_i_prob = tf.gather(
            tf.reshape(tf.nn.softmax(tf.reshape(self.all_logits, [1, -1])), [-1]), self.sup_ids)
        self.bce = tf.keras.losses.BinaryCrossentropy()
        self.sup_loss = self.bce(self.sup_labels, self.sup_i_prob) 
        
        #self.gan_loss = tf.Print(self.gan_loss, ['self.gan_loss=', self.gan_loss, '  '])
        ##add max diversity  ##if only considering topn diver, using self.dpp_i_embedding
        ...
        #self.det = -tf.log(1-tf.exp(-0.01*tf.linalg.det( self.item_kernel + tf.linalg.diag(tf.ones(self.len))*1e-3 )))  #sigma det of topn
        #self.dpp_norm = self.dpp_lamda * tf.nn.l2_loss(self.dpp_i_embedding)   #sigma det norm of topn i

        ##print tensors about dpp
        self.gen_loss =  self.sup_loss + self.gan_loss + self.norm_loss + self.dpp_loss_weight*(self.det + self.dpp_norm)
        ...
        # for test stage, self.u: [batch_size]     after updates
